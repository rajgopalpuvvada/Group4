    ...
    public function signup_user(Request $request)
    {
        ...
        if (isset($request->get->data)) {
            ...
            if (empty($data['jobRegNo'])) {
                $this->page->go_back("register", ["status" => "error", "message" => "Oops, Please fill in all the fields!"]);
                exit();
            }
    
        }

        $settings = new AppUtils(SettingsModel::$table);
        $settings_response = $settings->query_by_condition([
            'job_reg_numbers =' => $data['jobRegNo']
        ]);

        $user_util_response = $serve->query_by_condition([
            'jobRegNo =' => $data['jobRegNo']
        ]);

        if (count($settings_response) == 0 || count($user_util_response) > 0) {
            $this->page->go_back("register", [
                "status" => "error", "message" => "Oops Registration error. Please check your registration number, 
                it is either already registered with another person, no such Number known by the system, 
                or the number is not active; " . $data['jobRegNo'] . "!"
            ]);
            exit();
        }
